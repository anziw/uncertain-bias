...
results <- read.pcibex("results_gumball_vs_unk_election.csv")
```
...
dat <- dat %>%
  ...
  mutate(condition = ifelse(substring(trial_id, 1, 1) == "e", "unk_election", "gumball"),
         event_type = ifelse(substring(trial_id, 1, 1) == "e",
                       ifelse(substring(trial_id, 3, 3) == "x", "party_x", "party_y"),
                       ifelse(substring(trial_id, 3, 3) == "p", "purple_gumball", "orange_gumball")),
...
ggplot(subset(dat, condition == "gumball"), 
  ...
  geom_errorbar(aes(ymin=avg_prob - 2*se_prob,
                ...
                width = 2) +
  labs(title = "Usage of uncertainty expressions in gumball condition", 
       x = "Event likelihood", 
...
ggplot(subset(dat, condition == "unk_election"), 
       aes(x = event_likelihood, y = avg_prob, color = utterance)) +
  geom_line() + 
  geom_point() + 
  geom_errorbar(aes(ymin=avg_prob - 2*se_prob,
                    ymax=avg_prob + 2*se_prob),
                width = 2) +
  labs(title = "Usage of uncertainty expressions in unknown election condition", 
       x = "Event likelihood", 
       y = "Probability of using utterance") + 
  facet_grid(~event_type)
```
